...
- **Authentication** — `/api/v1/auth/*`: login, refresh (httpOnly cookie), logout. Access JWT carries **`sid`** (session id) and **`tv`** (token-version snapshot); middleware rejects stale **`tv`** vs Postgres. Staged enforcement: **`LUMOGIS_REQUIRE_TV_CLAIM`** (when **true**, missing **`tv`** → 401). **`GET/DELETE /api/v1/me/sessions`**, **`POST /api/v1/me/logout-all`**; admin session inventory under **`/api/v1/admin/users/{user_id}/sessions`**. Refresh cookie path **`/api/v1/auth`**. **`LUMOGIS_TOKEN_VERSION_CACHE_TTL_SECONDS`** bounds in-process version cache (single-worker / **`--workers 1`** supported; multi-worker coherence → **LUM-30**).
...
- **Context building** — retrieval + `context_budget` truncation **on plaintext fragments** before LLM calls (`ARCHITECTURE.md`, `services/context_budget.py`).
