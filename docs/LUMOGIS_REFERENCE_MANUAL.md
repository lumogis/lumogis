...
| **C** | Non-technical operator | **Lumogis Hub** bundled appliance (Core + Search UI, no Docker) | Search built into the appliance | N/A (local Core) | One appliance install — see Persona C row above; no separate Search download |
...
**Lumogis Hub (LUM-396, LUM-435):** the bundled **Persona C** appliance ships Core and **Lumogis Search** together without Docker — see the **Persona C** row in the [distribution matrix](#persona-a--b--c--distribution-matrix) above.
