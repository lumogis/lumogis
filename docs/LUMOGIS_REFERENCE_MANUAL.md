...
Verified against commit: **`6f3c5689b1f74e775ba66ea6dc734a77acfd6eba`** (`dev` — LUM-224 web Docker + LUM-250 `admin_users` 204; **ADR 043**) + **LUM-308** auto-RAG ship (**ADR 059**)
...
| `make doctor` | **LUM-199** — read-only host-side stack/config/network checks; JSON: **`make doctor ARGS="--json"`** (**`jq`** required); security audits opt-in: **`ARGS="--security"`** — see **`scripts/doctor/README.md`** |
