...
| **Desktop memory overlay** (`clients/lumogis-desktop/`, LUM-329) | **Shipped (v0.1, proprietary tree)** — bearer + keychain; **not** in public export — see **`clients/lumogis-desktop/README.md`** |
...
| `make desktop-dev` | **LUM-329** — Tauri overlay dev shell (`clients/lumogis-desktop/`) — requires Rust + Node 20 + OS webview deps — see **`clients/lumogis-desktop/README.md`** |
| `make desktop-build` | **LUM-329** — local `tauri build` parity (unsigned OK for smoke) |
| `make desktop-build-client-only` | **LUM-398** — same as **`desktop-build`** but merges **`tauri.client-only.conf.json`** (CI Persona B profile) |
