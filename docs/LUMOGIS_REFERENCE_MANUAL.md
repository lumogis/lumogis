...
**Notifications** — Daily digest patterns via ntfy and connector stack; **Web Push** product flows are **not** the same as the read-only **`/me/notifications`** settings façade (see §13). Architecture for a unified in-process **dispatcher**, per-user routing preferences, priority tiers, and server-side quiet-hours gate is decided in [ADR 077](decisions/077-lum-189-notification-architecture.md) (**planned** — implementation in **LUM-93** and follow-on tickets; **not shipped** in core yet).

**Implemented vs planned:** Core signals + digest + routines are **implemented** in the household self-hosted sense. Browser **Web Push** plumbing (**subscriptions**, minimal payloads, service-worker **`push`**/**`notificationclick`** handling with explicit opt-in from notifications settings) is **shipped**—see **§13**; pushing notifications for **every** connector/action outcome category remains **incomplete** compared to daily digest and **ntfy** paths. The **ntfy server** remains an optional max-privacy channel; the **ntfy mobile app is not required** — Web Push and in-app SSE cover foreground/background on Lumogis surfaces (native Tauri client is a separate downstream chunk).
...
| **Admin** | `/admin/users` (import/export, password reset), `/admin/connector-credentials`, `/admin/connector-permissions`, `/admin/mcp-tokens`, `/admin/audit`, `/admin/diagnostics`, **`/admin/system-status`** (live stack health — services, storage, Ollama read-only; **LUM-178**, [ADR 074](decisions/074-lum-178-stack-health-dashboard.md)) |
...
**Lumogis Search overlay (LUM-329, LUM-397, LUM-398, LUM-430):** the AGPL **Tauri 2** app under **`clients/lumogis-search/`** provides a **global hotkey** frameless overlay for **`GET /api/v1/memory/search`** with **native open/reveal** under configured **library roots** (optional for search — required only to open files locally; not auto-synced with server **`ingest_paths`**). First-run in-app onboarding: household server URL → **`GET /healthz`** → sign-in when auth is on. When **`AUTH_ENABLED=true`**, the overlay uses **email/password login**, stores a single **`session:{host-hash}`** keychain entry (JSON session: access, refresh, expiry, role), and **reactively refreshes** on **`401`** via **`POST /api/v1/auth/refresh`** (manual **`Set-Cookie`** parse — reqwest has no cookie jar; refresh sends **Bearer + refresh cookie** per server CSRF bypass). **Admins** edit **`ingest_paths`** via **`GET/PUT /settings`**, see **`paperless_configured`**, and can **push-upload** files via **`POST /api/v1/ingest/upload`**. Non-admin household members may still use push upload. Build: **`make search-dev`** / **`make search-build`**. See **`clients/lumogis-search/README.md`**. **Linux Wayland:** global shortcuts are not a v0.1 target.

**Lumogis Hub (LUM-396, LUM-435):** the **private** bundled Persona C appliance at **`apps/lumogis-hub/`** ships Core + Qdrant + Postgres + Ollama as Tauri sidecars with **no Docker**. Its built-in UI is **Lumogis Search** composed from the shared **`lumogis_search_lib`** Rust commands and **`@search-ui`** factory (Hub adds only **`ui/bundled/**`** and a thin **`ui/main.ts`**). Identifier **`com.lumogis.hub`**; keychain service unchanged (**`lumogis-overlay`**). Build: **`make hub-dev`** / **`make hub-build-bundled`** (maintainer; strip-listed from public export). See **`apps/lumogis-hub/README.md`**.
...
| **Phase 4** Web Push + notifications | **Partial** — subscription APIs + sender/service-worker handling shipped; unified dispatcher + prefs (**ADR 077**, **LUM-93+**) **planned not shipped**; **automatic push mirroring every connector/action outcome** not wired |
...
| `make hub-dev` | **LUM-435** — bundled Hub dev shell (`apps/lumogis-hub/`) — private maintainer tree |
| `make hub-build-bundled` | **LUM-435** — release bundled Hub (`cargo build --features bundled` + `tauri build` with bundled config) |
