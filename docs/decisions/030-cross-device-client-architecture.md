...
- 2026-05-27: **LUM-329 — Lumogis Search overlay (Tauri 2 v0.1)** shipped: system-wide hotkey overlay, **`GET /api/v1/memory/search`** consumer, bearer + OS keychain, native open/reveal behind **library roots**; **not** a Phase 6 full `lumogis-web` shell (cookie/SPA bundle) — that programme item stays **deferred** per LUM-44. Public AGPL export ships **`clients/lumogis-search/`** (**ADR 081**).
