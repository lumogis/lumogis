...
/// `generate_handler!` therefore registers 1 (local profile fn) + 19 (these) = 20 commands.
...
fn apply_onboarding_migration(cfg: &mut OverlayConfig, session_present_for_host: bool) -> bool {
...
pub fn shared_setup(
    ...
    let migrated = apply_onboarding_migration(&mut config, session_present);
    if migrated {
...
pub fn run() {
    shared_builder()
        .setup(|app| shared_setup(app.handle(), None))
...
mod command_registry_tests {
    ...
    /// exactly 19 with no duplicates. Search's `generate_handler!` registers these
    /// 19 plus the local `get_desktop_profile` = 20 total. This guards the
    /// frontend invoke-string contract, not macro registration.
    #[test]
    fn shared_command_names_count_and_unique() {
        assert_eq!(SHARED_COMMAND_NAMES.len(), 19, "shared command count drifted");
