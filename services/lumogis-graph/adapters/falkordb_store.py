...
class FalkorDBStore:
    ...
    def ping(self) -> bool:
        """Liveness probe at the Redis transport layer.

        Earlier versions ran ``ro_query("RETURN 1")`` against the configured
        graph key. On a fresh stack that key does not exist yet and FalkorDB
        replies ``Invalid graph operation on empty key`` — making `/health`
        report ``"falkordb": false`` until first ingest. Pinging the Redis
        connection avoids that false negative while still failing closed on
        DNS / TCP / auth errors.
        """
        from falkordb import FalkorDB  # type: ignore[import]

        try:
            return bool(FalkorDB(host=self._host, port=self._port).connection.ping())
        except Exception:
            return False
