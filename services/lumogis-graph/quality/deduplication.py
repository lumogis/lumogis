...
def _build_ann_blocks(
    ...
    # In the standalone lumogis-graph service the embedder/vector store may be
    # unconfigured (`None`) on purpose — Qdrant + sentence-transformers are
    # Core-side concerns. Skip ANN blocking entirely in that case; the
    # type-block and attr-block paths still produce candidates.
    embedder = config.get_embedder()
    if embedder is None or vs is None:
        _log.info(
            "dedup ANN: embedder or vector store unavailable — skipping ANN blocking "
            "(type-block + attr-block paths still active)"
        )
    ...
    for e in entities:
        ...
        # Search with explicit user_id filter — cross-user data leakage prevention
        try:
            results = vs.search(
                collection="entities",
                vector=vec,
...
def _route_pair(
    ...
    if prob >= _AUTO_MERGE_THRESHOLD:
        ...
        if mc_a >= _MIN_MENTION_COUNT_FOR_AUTO_MERGE and mc_b >= _MIN_MENTION_COUNT_FOR_AUTO_MERGE:
            ...
            # NOTE (lumogis-graph extraction): `services.entity_merge` is
            # Core-owned and is intentionally NOT vendored into this service
            # — only Core may DELETE rows from `entities`. The import below
            # will therefore raise ImportError in service mode, which the
            # outer except catches and routes the candidate into the review
            # queue. A follow-up work item ("dedup auto-merge callback to
            # Core") will replace this with an HTTP POST to Core's existing
            # `/entities/merge/deduplicate` endpoint so high-confidence
            # pairs auto-merge again instead of always queueing.
            try:
                from services.entity_merge import merge_entities  # type: ignore[import-not-found]
