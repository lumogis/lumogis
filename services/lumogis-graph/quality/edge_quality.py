...
INSERT … ON CONFLICT DO NOTHING (see orchestrator/services/entities.py). The
COUNT(DISTINCT evidence_id) guards in this module's PPMI queries are therefore
defence in depth, not the primary deduplication mechanism — they remain
because they correctly express the intent ("count each evidence once") even
if a future relaxation of the unique constraint were to permit duplicates
again.
...
def run_weekly_quality_job() -> dict:
    ...
    alias_violations = 0
    try:
        from quality.entity_constraints import check_orphan_entities
    ...
    try:
        from quality.entity_constraints import check_alias_uniqueness
    ...
    try:
        from quality.deduplication import run_deduplication_job
