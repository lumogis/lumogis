...
def test_build_filter_default_visibility_uses_should_or_union() -> None:
    ...
    assert b0.must[0] == FieldCondition(
        key="scope", match=MatchValue(value="personal")
    )
    ...
    assert b1.must[0] == FieldCondition(
        key="scope", match=MatchAny(any=["shared", "system"])
    )
...
def test_build_filter_entity_merge_style_must() -> None:
    flt = QdrantStore._build_filter(
        {"must": [{"key": "entity_id", "match": {"value": "e-1"}}]}
    )
