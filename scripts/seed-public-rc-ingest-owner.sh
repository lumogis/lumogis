...
#
# Set INBOX_OWNER_USER_ID in repo-root .env for RC ingest-path watcher tests.
...
if [[ "${COMPOSE_PROJECT_NAME:-}" != "lumogis-test" ]]; then
  echo "seed-public-rc-ingest-owner: refusing (COMPOSE_PROJECT_NAME=${COMPOSE_PROJECT_NAME:-<unset>}, expected lumogis-test)" >&2
  exit 2
fi
...
docker compose --env-file "$ROOT/$ENV_FILE" exec -T --workdir /project/orchestrator orchestrator \
  python -m scripts.seed_public_rc_ingest_owner
